 ...
 */
class BasicHcdHandlers(ctx: ActorContext[TopLevelActorMessage], cswCtx: CswContext) extends ComponentHandlers(ctx, cswCtx) {
  ...
  def onSetup(runId: Id, setup: Setup): SubmitResponse = {
    ...
    setup.commandName match {
      case `hcdSleep` =>
        ...
        cswCtx.timeServiceScheduler.scheduleOnce(when)(
          cswCtx.commandResponseManager.updateCommand(Completed(runId, Result().madd(resultKey.set(sleepTime))))
        )
        Started(runId)
      case `hcdImmediate` =>
        Completed(runId)
