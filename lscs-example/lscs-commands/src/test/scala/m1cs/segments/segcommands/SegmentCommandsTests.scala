...
import m1cs.segments.segcommands.Common.{ALL_SEGMENTS, AllActuators, segmentIdKey, segmentRangeKey, valuesToString}
...
class SegmentCommandsTests extends AnyFunSuite with Matchers {
  ...
  test("To From CFG_ACT_SNUM") {
    import m1cs.segments.segcommands.CFG_ACT_SNUB.*
    import m1cs.segments.segcommands.Common.ControllerModes.*

    // All 3 actuators
    var setup = toActSnubCtrl(prefix, AllActuators).asSetup
    toCommand(setup) shouldBe s"${COMMAND_NAME.name} ACT_ID=ALL"
    ...
    // All actuators + offload mode
    setup = toActSnubCtrl(prefix, AllActuators).withSnubberMode(CONTINUOUS).asSetup
    toCommand(setup) shouldBe s"${COMMAND_NAME.name} ACT_ID=ALL, MODE=CONTINUOUS"

    // All actuators + gain
    setup = toActSnubCtrl(prefix, AllActuators).withGain(5).asSetup
    toCommand(setup) shouldBe s"${COMMAND_NAME.name} ACT_ID=ALL, CONT_GAIN=5.0"

    // All actuators + thresh
    setup = toActSnubCtrl(prefix, AllActuators).withThresh(22.3).asSetup
    toCommand(setup) shouldBe s"${COMMAND_NAME.name} ACT_ID=ALL, CONT_THRESH=22.3"

    // All actuators + discreteStart
    setup = toActSnubCtrl(prefix, AllActuators).withDiscreteStart(101.23).asSetup
    toCommand(setup) shouldBe s"${COMMAND_NAME.name} ACT_ID=ALL, DISCRETE_START=101.23"

    // All actuators + discreteEnd
    setup = toActSnubCtrl(prefix, AllActuators).withDiscreteStop(-101.23).asSetup
    toCommand(setup) shouldBe s"${COMMAND_NAME.name} ACT_ID=ALL, DISCRETE_STOP=-101.23"
  ...
  test("To From SET_LIMIT_ACT") {
    ...
    // All 3 actuators
    var setup = toActPosLimit(prefix, AllActuators, ENCODER).asSetup
    ...
    setup = toActPosLimit(prefix, AllActuators, ENCODER).asSetup
    toCommand(setup) shouldBe s"${COMMAND_NAME.name} ACT_ID=ALL, SENSOR=ENCODER"

    // All actuators + upper limit
    setup = toActPosLimit(prefix, AllActuators, ENCODER).withUpperLimit(5).asSetup
    toCommand(setup) shouldBe s"${COMMAND_NAME.name} ACT_ID=ALL, SENSOR=ENCODER, POS_LIM=5.0"

    // All actuators + lower limit
    setup = toActPosLimit(prefix, AllActuators, SNUB).withLowerLimit(22.3).asSetup
  ...
  test("To From SET_PARAM_ACT") {
    ...
    // All 3 actuators
    var setup = toActMotorCharacteristics(prefix, AllActuators, VC).asSetup
    ...
    // All actuators + positionSenor
    setup = toActMotorCharacteristics(prefix, AllActuators, VC).asSetup
    toCommand(setup) shouldBe s"${COMMAND_NAME.name} ACT_ID=ALL, MOTOR=VC"

    // All actuators + coil
    setup = toActMotorCharacteristics(prefix, AllActuators, VC).withCoilResistance(5).asSetup
    toCommand(setup) shouldBe s"${COMMAND_NAME.name} ACT_ID=ALL, MOTOR=VC, COIL_RESISTANCE=5.0"

    // All actuators + operating
    setup = toActMotorCharacteristics(prefix, AllActuators, SNUB).withOperatingCurrent(22.3).asSetup
    toCommand(setup) shouldBe s"${COMMAND_NAME.name} ACT_ID=ALL, MOTOR=SNUB, OPER_CUR=22.3"

    // All actuators + standby
    setup = toActMotorCharacteristics(prefix, AllActuators, SNUB).withStandbyCurrent(13.8).asSetup
